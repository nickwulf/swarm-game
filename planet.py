...
class Planet:
  ...
  def draw(self):
    ...
    # Draw base planet
    pygame.gfxdraw.filled_circle(glob.windowSurface, self.pos[0], self.pos[1], int(self.size), self.player.color)
    if not isinstance(self.player, Neutral):
      barThickness = 5
      if self.population/self.maxPop >= 0.75:
        barThickness = 10-10*math.fabs((time.clock() % 1)-.5)
      util.drawPie(glob.windowSurface, self.pos, int(self.size), 270-180*(self.population/self.maxPop), 270+180*(self.population/self.maxPop), util.combineColors((255,255,255), self.player.color, .5))
      pygame.gfxdraw.filled_circle(glob.windowSurface, self.pos[0], self.pos[1], int(self.size-barThickness), self.player.color)
      pygame.gfxdraw.aacircle(glob.windowSurface, self.pos[0], self.pos[1], int(self.size-barThickness), self.player.color)
      if self.population/self.maxPop > 1:
        extraBar = 1-math.exp(-math.log(2.0)*(self.population/self.maxPop-1))
        util.drawPie(glob.windowSurface, self.pos, int(self.size), 90-180*extraBar, 90+180*extraBar, util.combineColors((255,255,255), self.player.color, barThickness/10))
        extraBarThickness = 10
        pygame.gfxdraw.filled_circle(glob.windowSurface, self.pos[0], self.pos[1], int(self.size-extraBarThickness), self.player.color)
        pygame.gfxdraw.aacircle(glob.windowSurface, self.pos[0], self.pos[1], int(self.size-extraBarThickness), self.player.color)
    pygame.gfxdraw.aacircle(glob.windowSurface, self.pos[0], self.pos[1], int(self.size), (0,0,0))
